jQuery(function ($) {
	...
	var WPMake_Advance_User_Avatar_Frontend = {
		init: function () {
			WPMake_Advance_User_Avatar_Frontend.process_avatar_upload();
		},
		/**
						...
						if (
							...
							$(".swal2-cancel ").on("click", function () {
								$(".wpmake-advance-user-avatar-upload")
									.find("#wpmake-advance-user-avatar-pic")
									.val("");
							});
							WPMake_Advance_User_Avatar_Frontend.crop_image(
								$(this)
							);
						} else {
							WPMake_Advance_User_Avatar_Frontend.send_file(
								$(this)
							);
						}
					}
		...
		crop_image: function (file_instance) {
			...
			$(".swal2-confirm").on("click", function () {
				...
				WPMake_Advance_User_Avatar_Frontend.send_file(file_instance);
			});
		},
		remove_avatar: function ($node) {
			...
			$.ajax({
				...
				complete: function (ajax_response) {
					$node
						.closest(".wpmake-advance-user-avatar-upload")
						.find(".wpmake-advance-user-avatar-remove")
		...
		send_file: function ($node) {
			...
			if ($node[0].files[0]) {
				...
				var blob =
					WPMake_Advance_User_Avatar_Frontend.dataURItoBlob(img);
