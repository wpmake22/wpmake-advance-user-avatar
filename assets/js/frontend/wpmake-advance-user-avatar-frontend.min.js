jQuery(function (i) {
	...
	var n = {
				...
				function () {
					...
					this.files &&
						...
						(((a = new FileReader()).onload = function (a) {
							i(".img").attr("src", a.target.result);
						}),
						...
						wpmake_advance_user_avatar_params.wpmake_advance_user_avatar_enable_cropping_interface
							? (Swal.fire({
							  ...
							  i(".swal2-cancel ").on("click", function () {
									i(".wpmake-advance-user-avatar-upload")
		...
		crop_image: function (e) {
			var r;
			i("#crop_container").Jcrop({
				aspectRatio: 1,
				onSelect: function (a) {
					r = { x: a.x, y: a.y, w: a.w, h: a.h };
				},
				setSelect: [100, 100, 50, 50],
			}),
				i(".swal2-confirm").on("click", function () {
					var a = {
						x: r.x,
						y: r.y,
						w: r.w,
						h: r.h,
						holder_width: i("#crop_container").css("width"),
						holder_height: i("#crop_container").css("height"),
					};
					i(".cropped_image_size").val(JSON.stringify(a)),
						n.send_file(e);
				});
		},
		...
		send_file: function (c) {
			var a =
				...
				_ =
					(wpmake_advance_user_avatar_params.wpmake_advance_user_avatar_enable_cropping_interface
						? (r = i("#crop_container").attr("src"))
						: ((r = i("#blob_container").attr("src")),
						  console.log("2")),
					c[0].files[0]
						? e.append("file", c[0].files[0])
						: ((r = n.dataURItoBlob(r)),
						  (r = new File([r], "snapshot.jpg")),
						  e.append("file", r)),
					e.append("cropped_image", i(".cropped_image_size").val()),
					e.append(
						"valid_extension",
						i('input[name="profile-pic"]').attr("accept")
					),
					e.append(
						"max_uploaded_size",
						i('input[name="profile-pic"]').attr("size")
					),
					c
						.closest(".wpmake-advance-user-avatar-upload")
			...
			i.ajax({
				...
				complete: function (a) {
					...
					try {
						...
						if (
							"undefined" == typeof n.success ||
							"undefined" == typeof n.data
						)
						...
						(r = n.data.message),
							...
							n.success &&
								((r = ""),
								(t = n.data.attachment_id),
								(e = n.data.profile_picture_url),
								i(".wpmake-advance-user-avatar-container")
		...
		dataURItoBlob: function (a) {
			for (
				var e = (
						0 <= a.split(",")[0].indexOf("base64") ? atob : unescape
					)(a.split(",")[1]),
	...
	n.init(jQuery),
		...
		i(document).on(
			...
			function () {
				var e = i(this),
					...
					r = 240,
					t =
						(i(window).width() < i(window).height() &&
							((a = 240), (r = 320)),
						Webcam.set({
							width: a,
							height: r,
		...
		i(document).on(
			"click",
			".wpmake-advance-user-avatar-remove",
			function () {
